...
def check_password():
    ...
    if "password_correct" not in st.session_state:
        st.text_input("Senha", type="password", on_change=password_guessed, key="password")
        return False
    return st.session_state["password_correct"]

if check_password():
    # TODO O SEU CÓDIGO DO DASHBOARD VAI AQUI
    ...
    if not df_filtrado.empty:
        ...
        salario_medio, salario_maximo, total_registros, cargos_mais_frequentes = 0, 0, 0, ""
    
    col1, col2, col3, col4 = st.columns(4)
    col1.metric("Salário Médio", f"${salario_medio:,.2f}")
    col2.metric("Salário Máximo", f"${salario_maximo:,.2f}")
    col3.metric("Total de Registros", f"{total_registros:,}")
    col4.metric("Cargo Mais Frequente", cargos_mais_frequentes)
